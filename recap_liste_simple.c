...
Cafenea* copiereInvector(Nod* lista, int pragNrLocuri, int* n) {
	*n = 0;
	Nod* aux = lista;
	while (aux != NULL) {
		if (aux->info.nrLocuri >= pragNrLocuri) {
			(*n)++;
		}
		aux = aux->next;
	}
	if (*n == 0) {
		...
		int k = 0;
		while (lista != NULL) {
			if (lista->info.nrLocuri >= pragNrLocuri) {
...
void dezalocareListaCirculara(Nod** lista) {
	if ((*lista) == NULL) {
		printf("NU");
	}
	Nod* aux = (*lista)->next;
