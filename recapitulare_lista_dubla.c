...
void citireDinFisier(const char* numeFisier, ListaDubla* lista) {
...
void inserareSortataListaCirculara(ListaDubla* lista, Cafenea c) {
	...
	if (lista->cap == NULL) {
		...
		if (lista->cap->info.nrLocuri > c.nrLocuri) {
			...
			Nod* aux = lista->cap;
			while (aux->urmator != NULL && aux->urmator->info.nrLocuri < c.nrLocuri) {
				aux = aux->urmator;
			}
			if (aux == lista->coada) {
				nou->anterior = lista->coada;
...
void afisareInversCirculara(ListaDubla lista) {
	Nod* aux = lista.coada;
	if (aux == NULL) {
		printf("NU");
	}
...
void dezalocareCirculara(ListaDubla* lista) {
	Nod* temp = lista->cap;
	Nod* cap = temp;
	do {
...
void main() {
	...
	citireDinFisier("cafenele.txt", &listaFisier);
	...
	inserareSfarsitListaCirculara(&listaCirculara, cafenea3);
	afisareListaCirculara(listaCirculara);
	printf("\n");
	afisareInversCirculara(listaCirculara);

	dezalocareCirculara(&listaCirculara);
