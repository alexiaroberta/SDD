...
//parcurgere in latime - coada
void afisareParcurgereInLatime(NodPrincipal* graf, int serieStart) {
	if (graf) {
		Nod* coada = NULL;
...
void afisareParcurgereInAdancime(NodPrincipal* graf, int serieStart) {
	if (graf) {
		Nod* stiva = NULL;
		int nrNoduri = calculNrNoduri(graf);
		char* vizitat = (char*)malloc(sizeof(char) * nrNoduri);
...
void main() {
	...
	graf = inserarePrincipala(graf, initCamion(4, "Scania", 20));
	graf = inserarePrincipala(graf, initCamion(3, "Volvo", 30));
	graf = inserarePrincipala(graf, initCamion(2, "Renault", 25));
	graf = inserarePrincipala(graf, initCamion(1, "MAN", 15));
	graf = inserarePrincipala(graf, initCamion(0, "Mercedes", 40));
	...
	afisareParcurgereInLatime(graf, 4);
	printf("\n\n");
	afisareParcurgereInAdancime(graf, 4);
