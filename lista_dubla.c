...
void inserareInceputListaCirculara(ListaDubla* lista, Cafenea c) {
	...
	aux->info = initCafenea(c.nume, c.nrLocuri, c.suprafata);
	if (lista->cap != NULL) {
		aux->anterior = lista->coada;
		aux->urmator = lista->cap;
		lista->cap->anterior = aux;
		lista->coada->urmator = aux;
		lista->cap = aux;
	}
	else {
		aux->anterior = aux;
...
void inserareSortataListaCirculara(ListaDubla* lista, Cafenea c) {
	...
	if (lista->cap == NULL) {
		...
		if (lista->cap->info.nrLocuri > c.nrLocuri) {
			...
			Nod* aux = lista->cap;
			while (aux->urmator != lista->cap && aux->urmator->info.nrLocuri < c.nrLocuri) {
				aux = aux->urmator;
			}
			if (aux == lista->coada) {
				nou->anterior = lista->coada;
...
void afisareInversListaCirculara(ListaDubla lista) {
	Nod* aux = lista.coada;
	if (aux == NULL) {
		printf("NU");
	}
...
void main() {
	...
	int nr;
	Cafenea* vector = copiazaInVector(lista, 100, &nr);
	if (vector != NULL) {
		for (int i = 0; i < nr; i++) {
	...
	printf("\n");
	afisareInversListaCirculara(listaCirculara);
